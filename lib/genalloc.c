				     ...
				     unsigned alignment_order)
{
	...
	rcu_read_lock();
	list_for_each_entry_rcu(chunk, &pool->chunks, next_chunk) {
		unsigned long local_chunk_size;
		if (size > atomic_read(&chunk->avail))
			continue;
		local_chunk_size = chunk_size(chunk) >> order;

retry:
		start_bit = bitmap_find_next_zero_area_off(chunk->bits, local_chunk_size,
						   0, nbits, align_mask,
						   chunk->start_addr >> order);
		if (start_bit >= local_chunk_size)
			continue;
