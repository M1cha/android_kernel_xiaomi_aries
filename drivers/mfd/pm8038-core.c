		      ...
		      struct pm8038 *pmic)
{
	...
	if (pdata->leds_pdata) {
		...
		ret = mfd_add_devices(pmic->dev, 0, &leds_cell, 1, NULL, 0, NULL);
		if (ret) {
	...
	if (pdata->vibrator_pdata) {
		...
		ret = mfd_add_devices(pmic->dev, 0, &vibrator_cell, 1, NULL, 0, NULL);
		if (ret) {
	...
	if (pdata->spk_pdata) {
		...
		ret = mfd_add_devices(pmic->dev, 0, &spk_cell, 1, NULL, 0, NULL);
		if (ret) {
