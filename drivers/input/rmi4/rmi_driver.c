...
static int rmi_driver_probe(struct rmi_device *rmi_dev)
{
	...
	retval = rmi_read_block(rmi_dev,
				data->f01_container->fd.query_base_addr + 11,
				product_id,
				RMI_PRODUCT_ID_LENGTH);
	if (retval < 0)
		dev_warn(&fc->dev, "Failed to get product id\n");
	product_id[RMI_PRODUCT_ID_LENGTH] = 0;
	rmi_dev->product_id = kzalloc(RMI_PRODUCT_ID_LENGTH+1, GFP_KERNEL);
	if (!rmi_dev->product_id)
		goto err_free_data;
	memcpy(rmi_dev->product_id, product_id, RMI_PRODUCT_ID_LENGTH+1);
