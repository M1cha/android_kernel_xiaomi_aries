...
static void WriteInitialRegisterValues ( void )
{
	...
	I2C_WriteByte(PAGE_0_0X72, 0x91, 0xA5);// Clear bit 6 (reg_skip_rgnd) set to  push_pull atate
	I2C_WriteByte(PAGE_0_0X72, 0x95, 0x71);
#else
	// Do not perform RGND impedance detection if connected to SiI 9290
	I2C_WriteByte(PAGE_0_0X72, 0x91, 0xAD);// Clear bit 6 (reg_skip_rgnd)
...
static void	SwitchToD3( void )
{
	if(POWER_STATE_D3 != fwPowerState)
	{
		...
		I2C_WriteByte(PAGE_0_0X72, 0x74, 0xBF);  // keep BIT6 for fast plug case
...
static int Int4Isr( void )
{
	...
	I2C_WriteByte(PAGE_0_0X72, (0x74), reg74&0xFE);	// clear all interrupts except bit0
...
#define CBUS_LINK_STATUS_2 0x38
	...
	//if ((cbusInt & BIT3)||timer_SWWA_WRITE_STAT_Expired)
	{
		uint8_t status[4];
		uint8_t address;
