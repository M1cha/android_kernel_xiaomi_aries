							...
							int chargecycles)
{
	...
	if (chip->adjusted_fcc_temp_lut == NULL) {
		...
		scalefactor = interpolate_scalingfactor_fcc(chip->fcc_sf_lut, chargecycles);
							...
							int chargecycles)
{
	...
	pc = interpolate_pc(chip->pc_temp_ocv_lut, batt_temp / 10, ocv_uv / 1000);
				...
				int *ret_pc_unusable)
{
	...
	for (i = 0; i <= 100; i++) {
		ocv_mv = interpolate_ocv(chip->pc_temp_ocv_lut, batt_temp_degc, i);
			...
			int *iavg_ma)
{
	...
	new_unusable_mv = interpolate_ocv(chip->pc_temp_ocv_lut, batt_temp_degc,
						chip->prev_pc_unusable);
